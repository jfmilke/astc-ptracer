#ifndef JAY_GRAPHICS_RENDERER_HPP
#define JAY_GRAPHICS_RENDERER_HPP
...
namespace jay
{
class JAY_EXPORT renderer : public system
{
public:
  renderer           ();
 ...
 ~renderer           ()                      = default;
  ...
  {
    static_assert(std::is_base_of<jay::render_pass, render_pass_type>::value, "The type does not inherit from render_pass.");
...
protected:
  ...
  static bool       render_pass_type_predicate(const std::unique_ptr<jay::render_pass>& iteratee)
  {
    return typeid(system_type) == typeid(*iteratee.get());
  }

  std::vector<std::unique_ptr<jay::render_pass>> render_passes_;
};
}
