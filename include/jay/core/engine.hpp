#ifndef JAY_CORE_ENGINE_HPP
#define JAY_CORE_ENGINE_HPP
...
public:
  ...
  {
    static_assert(std::is_base_of<jay::system, system_type>::value, "The type does not inherit from system.");
...
protected:
  template<typename system_type>
  static bool  system_type_predicate(const std::unique_ptr<jay::system>& iteratee)
  {
    return typeid(system_type) == typeid(*iteratee.get());
  }

  std::vector<std::unique_ptr<jay::system>> systems_;
};
}
