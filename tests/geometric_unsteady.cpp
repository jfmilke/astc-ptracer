...
void compress_custom(std::string filename, std::vector<astcenc_image*> data_imgs, std::vector<float>& peaks, jay::io& filedriver, jay::astc& astc_compressor, astcenc_preset preset, glm::ivec3 blocksize, bool per_channel_normalization, bool mask)
{
  ...
  descriptions.push_back("Compress File");
  auto t1 = std::chrono::high_resolution_clock::now();
  timings.push_back((double)std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count());
...
TEST_CASE("Encoder.", "[jay::engine]") 
{
  ...
  for (auto n = 0; n < 2; n++)
    for (auto i = 0; i < input_filenames.size(); i++)
    {
      ...
      descriptions.push_back("Load File");
      auto t1 = std::chrono::high_resolution_clock::now();
      timings.push_back((double)std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count());
      ...
      descriptions.push_back("Find Peaks");
      t1 = std::chrono::high_resolution_clock::now();
      timings.push_back((double)std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count());

      // Convert data
      t0 = std::chrono::high_resolution_clock::now();
      auto imgs = (n) ? astc_compressor.convert_data_to_img_refined(dataset, false, peaks, jay::colorspace::RGB, jay::slicetype::Plane, 0) :
        astc_compressor.convert_data_to_img(dataset, false, peaks, jay::colorspace::RGB, jay::slicetype::Plane, 0);
      descriptions.push_back("Convert Data");
      t1 = std::chrono::high_resolution_clock::now();
      timings.push_back((double)std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count());

      // Store peaks
      filedriver.store_vector(peaks, output_path + filenames[i] + ".peaks", 1);
  ...
  jay::data_io::store_descriptive_text("Compression Timings", descriptions, timings, output_path + "Timings.txt", 1, 1);
