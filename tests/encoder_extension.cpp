...
void compress_custom(std::string filename, std::vector<astcenc_image*> data_imgs, std::vector<float>& peaks, jay::io& filedriver, jay::astc& astc_compressor, astcenc_preset preset, glm::ivec3 blocksize, bool per_channel_normalization, bool mask)
{
  ...
  timings.push_back((std::size_t)std::chrono::duration_cast<std::chrono::seconds>(t1 - t0).count());
...
TEST_CASE("Encoder.", "[jay::engine]")
{
      ...
      for (auto m = 1; m < 2; m++)
        for (auto p = 3; p < 4; p++)
        {
          ...
          */
          ...
          {
            jay::astc astc_compressor4 = jay::astc();
            compress_custom(filenames[i], imgs, peaks, filedriver, astc_compressor4, astcenc_preset(p), glm::ivec3(12, 12, 1), n, m);
            astc_compressor4.free_contexts();
          }

          jay::data_io::store_descriptive_text("Compression Timings", descriptions, timings, output_path + "Timings_Extension" + std::to_string(counter) + ".txt", 1, 1);
          counter++;
        }
  ...
  jay::data_io::store_descriptive_text("Compression Timings", descriptions, timings, output_path + "Timings_Extension.txt", 1, 1);
