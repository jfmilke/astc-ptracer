...
TEST_CASE("Encoder.", "[jay::engine]") 
{
      ...
      for (auto m = 0; m < 1; m++)
        for (auto p = 0; p < 4; p++)
        {
          ...
          */
          ...
          jay::data_io::store_descriptive_text("Compression Timings", descriptions, timings, output_path + "Timings_Minimum" + std::to_string(counter) + ".txt", 1, 1);
          counter++;
        }
  ...
  jay::data_io::store_descriptive_text("Compression Timings", descriptions, timings, output_path + "Timings_Minimum.txt", 1, 1);
